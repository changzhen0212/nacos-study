...
public class Cluster extends com.alibaba.nacos.api.naming.pojo.Cluster implements Cloneable {
    ...
    public List<Instance> allIPs() {
        ...
        // # Set<> ephemeralInstances 注册的实例都在这里
     ...
     *                  # 传过来的注册的实例,包含新的实例和旧的实例
     * @param ephemeral whether these instances are ephemeral
     */
    // ! 写时复制的思想
    public void updateIps(List<Instance> ips, boolean ephemeral) {
        // # ephemeral 默认true AP架构
        // # ips- 传过来的注册的实例,包含新的实例和旧的实例, ephemeralInstances- 已经存在的实例,可能有很多服务了
        Set<Instance> toUpdateInstances = ephemeral ? ephemeralInstances : persistentInstances;
        // # 创建一个map
        HashMap<String, Instance> oldIpMap = new HashMap<>(toUpdateInstances.size());
        // # 遍历复制过来的内存中的注册实例, 放入新的map
        for (Instance ip : toUpdateInstances) {
            oldIpMap.put(ip.getDatumKey(), ip);
        }
        // # 下面逻辑操作的都是ips 用副本 oldIpMap 与传进来的 ips做比对, 如果没有就添加, 有则做一些修改
        // # 修改,是把改动的instance对象传入,将副本中的对象做替换,并不是直接在副本中做修改
        ...
        // # 最终覆盖原来的Set, 操作的是ips,传进来的 ips, 并不是那个副本
        toUpdateInstances = new HashSet<>(ips);

        if (ephemeral) {
            // # Set<Instance> ephemeralInstances 注册表的一部分 ,
            // # 回到注册表中可以查看 ServiceManager--> serviceMap--> Service--> Cluster
            // # 把副本替换系统中的注册表
